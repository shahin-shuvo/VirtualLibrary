package com.sasam.virtuallibrary.ChatRoom.service;
...
import com.sasam.virtuallibrary.ChatRoom.model.Friend;
import com.sasam.virtuallibrary.ChatRoom.model.ListFriend;
...
public class ServiceUtils {
    ...
    public static void userGoesOffline(Context context){
        if(isNetworkConnected(context)) {
            String uid = SharedPreferenceHelper.getInstance(context).getUID();
            if (!uid.equals("")) {
                FirebaseDatabase.getInstance().getReference().child("user/" + uid + "/status/isOnline").setValue(false);
                FirebaseDatabase.getInstance().getReference().child("user/" + uid + "/status/timestamp").setValue(System.currentTimeMillis());
            }
        }

        Log.d(String.valueOf(R.string.Logger),"offline");
    }
